...
//   };
//   useEffect(()=>{
//     const response = axios.get('http://localhost:5000/users').then((response) => {
//       setEndUser(response.data.filter((users) => user.email === users.email)[0]._id);
//     });
//   },[user])
//   console.log(endUser)

//   const userPhotoURL = user?.photoURL || ''; // Use an empty string if photoURL is null
//   const userDisplayName = user?.displayName || ''; // Use an empty string if photoURL is null
//   const SectionContainer = styled(Box)(({ theme }) => ({
//     margin: `${theme.spacing(3)} 0`,
//     padding: theme.spacing(2),
//     border: '1px solid #ccc',
